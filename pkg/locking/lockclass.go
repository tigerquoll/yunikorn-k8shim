...
//
// The classes are the ones the "+lockclass" annotations name, so that the runtime check and the
// static analysis enforce one declaration rather than two; anything that carries no annotation
// stays classless. A class without an edge in the "+lockorder" taxonomy of locking.go is only
// checked against itself. The order itself and the checking live in lockclass_deadlock.go: the
// check is only compiled into the deadlock tagged build so that the default build keeps the
// plain, inlinable forwarders, see forwarders.go.
type Class uint32
...
var className = [numClasses]string{
	...
	ClassWebhookManager:     "admission.WebhookManager",
}
