...
// shape of the errorBuf the core locking package installs, with a plain sync.Mutex because a
// wrapper lock of pkg/locking here would be taken from inside the check that is being reported.
...
func startClassOrderCheck(t *testing.T) *reportBuf {
	t.Helper()
...
func TestLockClassOrderEdges(t *testing.T) {
	tests := []struct {
		...
		{
			// the taxonomy has the application lock before the task lock
			name: "application under task",
			violate: func(_ *Context, app *Application, task *Task, _ *external.SchedulerCache) {
				task.lock.Lock()
				app.lock.Lock()
				app.lock.Unlock() //nolint:staticcheck // SA2001: the critical section is empty on purpose, the acquisition is what is checked
				task.lock.Unlock()
			},
			expected: "acquiring cache.Application while holding cache.Task",
		},
		{
			// the taxonomy has the context lock before the application lock
			name: "context under application",
			violate: func(context *Context, app *Application, _ *Task, _ *external.SchedulerCache) {
				app.lock.Lock()
				context.lock.Lock()
				context.lock.Unlock() //nolint:staticcheck // SA2001: the critical section is empty on purpose, the acquisition is what is checked
				app.lock.Unlock()
			},
		...
		},
		{
			...
			violate: func(_ *Context, _ *Application, task *Task, schedulerCache *external.SchedulerCache) {
				...
				task.lock.Lock()
				task.lock.Unlock() //nolint:staticcheck // SA2001: the critical section is empty on purpose, the acquisition is what is checked
...
// avoids by starting a goroutine, see the note on the taxonomy in pkg/locking/locking.go.
func TestLockClassPlaceholderManagerUnordered(t *testing.T) {
	...
	t.Run("manager under application", func(t *testing.T) {
		...
		app.lock.Lock()
		manager.Lock()
		manager.Unlock() //nolint:staticcheck // SA2001: the critical section is empty on purpose, the acquisition is what is checked
		app.lock.Unlock()
		assertNoOrderViolation(t, reports)
	})
}
...
func assertNoOrderViolation(t *testing.T, reports *reportBuf) {
	t.Helper()
